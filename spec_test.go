...
func TestMapSpec(t *testing.T) {
	testCases := map[string]struct {
		...
		"WithParams": {
			...
			Comments: map[string]string{
				"KEY1": `number!:{"length":10}`,
				"KEY2": `number!:{"length":9}`,
			},
			Expected: Specs{
				"KEY1": {Name: SpecNameNumber, Required: true, Valid: true},
				"KEY2": {Name: SpecNameNumber, Required: true},
			},
		},
	}
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			specs := GenerateSpecsFromComments(tc.Values, tc.Comments)
